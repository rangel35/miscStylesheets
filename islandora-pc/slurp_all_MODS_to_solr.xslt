...
<xsl:stylesheet version="1.0" xmlns:java="http://xml.apache.org/xalan/java"
  ...
  <xsl:include
    href="/var/lib/tomcat7/webapps/fedoragsearch/WEB-INF/classes/fgsconfigFinal/index/FgsIndex/islandora_transforms/library/xslt-date-template.xslt"/>
  ...
  <xsl:include
    href="/var/lib/tomcat7/webapps/fedoragsearch/WEB-INF/classes/fgsconfigFinal/index/FgsIndex/islandora_transforms/manuscript_finding_aid.xslt"/>
  ...
  <xsl:template match="mods:name[mods:role/mods:roleTerm]" mode="slurping_MODS">
    ...
    <xsl:for-each select="mods:role/mods:roleTerm">
      <xsl:variable name="this_prefix"
        select="concat($base_prefix, translate(normalize-space(.), $uppercase, $lowercase), '_')"/>
    ...
    <xsl:if test="@authority">
      <xsl:call-template name="general_mods_field">
        <xsl:with-param name="prefix"
          select="concat($prefix, 'authority_', translate(@authority, $uppercase, $lowercase), '_')"/>
        <xsl:with-param name="suffix" select="$suffix"/>
        <xsl:with-param name="value" select="$value"/>
        <xsl:with-param name="pid" select="$pid"/>
        <xsl:with-param name="datastream" select="$datastream"/>
  ...
  <xsl:template name="mods_language_fork">
    ...
    <xsl:if test="@lang">
      <xsl:call-template name="mods_authority_fork">
        <xsl:with-param name="prefix"
          select="concat($prefix, 'lang_', translate(@lang, $uppercase, $lowercase), '_')"/>
        <xsl:with-param name="suffix" select="$suffix"/>
        <xsl:with-param name="value" select="$value"/>
        <xsl:with-param name="pid" select="$pid"/>
        <xsl:with-param name="datastream" select="$datastream"/>
  ...
  <xsl:template name="general_mods_field">
    ...
    <xsl:for-each select="mods:relatedItem[@displayLabel]/mods:titleInfo/mods:title[normalize-space(text())]">
      <!--don't bother with empty space-->
      <field>
        <xsl:attribute name="name">
          <xsl:value-of select="concat('mods_relatedItem_displayLabel_', translate(../../@displayLabel, ' ', '_'), '_', $suffix)" />
        </xsl:attribute>
    ...
    <xsl:for-each select="mods:subject[@displayLabel]/*[normalize-space(text())]">
      ...
      <field>
        <xsl:attribute name="name">
          <xsl:value-of select="concat('mods_subject_displayLabel_', translate(../@displayLabel, ' ', '_'), '_', $suffix)"/>
        </xsl:attribute>
        <xsl:value-of select="text()"/>
      </field>
    </xsl:for-each>
    ...
    <xsl:if test="normalize-space($node/@displayLabel)">
          ...
          <field>
            <xsl:attribute name="name">
              <!--     <genre authority="gmgpc" authorityURI="http://www.loc.gov/pictures/collection/tgm/" valueURI="http://www.loc.gov/pictures/item/tgm007068/" displayLabel="Format" lang="eng">Newspapers</genre>
                        mods_genre_lang_eng_authority_gmgpc__Newspapers_displayLabel_mt
                     <genre displayLabel="Formato" lang="spa">Periódicos</genre>
                        mods_genre_lang_spa__Periódicos_displayLabel_ms  -->

              <xsl:value-of select="concat($prefix, 'displayLabel_', translate(@displayLabel, ' ', '_'), '_', $suffix)" />
            </xsl:attribute>
            <xsl:value-of select="text()"/>
          </field>
    </xsl:if>
